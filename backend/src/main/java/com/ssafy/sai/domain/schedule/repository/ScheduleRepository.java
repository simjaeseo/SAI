...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
    ...
    // 교육생 최근 일정 조회
    @Query(value = "select new com.ssafy.sai.domain.schedule.dto.ScheduleSinceTodayByStudentResponse(s.id, s.scheduleDate, s.startTime, s.endTime, s.category, s.detail, m.id, m.name) from Schedule as s left outer join Member as m on s.memberConsultant.id = m.id where s.memberStudent.id = :memberId and s.scheduleDate >= :today order by s.scheduleDate asc, s.startTime asc")
    List<ScheduleSinceTodayByStudentResponse> selectScheduleSinceTodayByStudent(@Param("memberId") Long memberId, @Param("today")LocalDate today, Pageable pageable);

    // 컨설턴트 최근 일정 조회
    @Query(value = "select new com.ssafy.sai.domain.schedule.dto.ScheduleSinceTodayByConsultantResponse(s.id, s.scheduleDate, s.startTime, s.endTime, s.category, s.detail, m.id, m.name) from Schedule as s left outer join Member as m on s.memberStudent.id = m.id where s.memberConsultant.id = :memberId and s.scheduleDate >= :today order by s.scheduleDate asc, s.startTime asc")
    List<ScheduleSinceTodayByConsultantResponse> selectScheduleSinceTodayByConsultant(@Param("memberId") Long memberId, @Param("today")LocalDate today, Pageable pageable);
